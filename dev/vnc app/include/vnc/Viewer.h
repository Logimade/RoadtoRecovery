...
typedef struct {
  /**
   * Notification that the server's mouse pointer has changed position. This
   * callback is optional.
   * Note that this callback will only be triggered when connected to SDK
   * Server endpoints, RealVNC Connect support coming soon.
   *
   * @param x The X coordinate.
   * @param y The Y coordinate.
   * @param local Best-effort indication of movement origin - true = server.
   * 
   * @since 1.10
   */
  void
  (*serverPointerPos)(void* userData,
    vnc_Viewer* viewer,
    int x, int y, vnc_bool_t local);

  /**
   * Notification that the server's pointer image has changed.  This callback is
   * optional.
   *
   * Upon receiving this callback, call vnc_Viewer_getServerPointerFbData()
   * 
   * @param x The X offset of the "hotspot" of the pointer.
   * @param y The Y offset of the "hotspot" of the pointer.
   * @param width The width of the pointer framebuffer.
   * @param height The height of the pointer framebuffer.
   *
   * @since 1.10
   */
  void
  (*serverPointerImageChanged)(void* userData,
    vnc_Viewer* viewer,
...
vnc_Viewer_setServerPointerCallback(
  vnc_Viewer* viewer,
  _In_opt_ const vnc_Viewer_ServerPointerCallback* callback,
  void* userData);

/**
 * Sets the pixel format of the pointer framebuffer. If not set, the pixel format
 * will be vnc_PixelFormat_rgb888.
 *
 * @param viewer The Viewer.
 * @param pf Pixel format for the pixel data.
 *
 ...
 */
VNC_SDK_API void
vnc_Viewer_setServerPointerFbPixelFormat(vnc_Viewer* viewer, const vnc_PixelFormat* pf);

/**
 * Sets the rate with which serverPointerPos callback will be called.
 *
 * @param viewer The Viewer.
 * @param periodMs The period in milliseconds between serverPointerPos calls.
 *
 * @since 1.10
 */
VNC_SDK_API void vnc_Viewer_setServerPointerPosPeriod(vnc_Viewer* viewer, int periodMs);

/**
 * Gets the width of the pointer framebuffer.
 */
VNC_SDK_API int vnc_Viewer_getServerPointerFbWidth(vnc_Viewer* viewer);

/**
 * Gets the height of the pointer framebuffer.
 */
VNC_SDK_API int vnc_Viewer_getServerPointerFbHeight(vnc_Viewer* viewer);

/**
 * Gets the X offset of the "hotspot" of the pointer.
 */
VNC_SDK_API int vnc_Viewer_getServerPointerXOffset(vnc_Viewer* viewer);

/**
 * Gets the Y offset of the "hotspot" of the pointer.
 */
VNC_SDK_API int vnc_Viewer_getServerPointerYOffset(vnc_Viewer* viewer);

/**
 * Gets the pixel format of the pointer framebuffer.
 */
VNC_SDK_API const vnc_PixelFormat*
vnc_Viewer_getServerPointerFbPixelFormat(vnc_Viewer* viewer);

/**
 * Gets the current mouse cursor image from the Server for rendering locally.
 * The returned object is valid until the next call to
 ...
 */
VNC_SDK_API const vnc_DataBuffer*
vnc_Viewer_getServerPointerFbData(vnc_Viewer* viewer);
